...
CADA ORDEN EXIGE LA PALABRA CLAVE, SIN EXCEPCION. Se probo una ventana de unos
segundos tras cada orden en la que no hacia falta repetirla, con una allowlist
de acciones reversibles y sin LLM como filtro. En uso real no compensa: el
filtro deja pasar poco de lo que quieres y algo de lo que no, y depender de que
el asistente adivine si le hablabas a el sale mucho mas caro que decir "Apolo"
otra vez. La palabra clave es la unica senal fiable de que la frase iba
dirigida al asistente, y aqui no se sustituye por ninguna heuristica.
"""
